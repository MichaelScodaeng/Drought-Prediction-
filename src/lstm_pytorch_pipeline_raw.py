...
try:
    ...
    # NOTE: We are NOT importing engineer_features for this pipeline
    print("LSTM Raw Pipeline: Successfully imported utility functions.")
except ImportError as e:
    print(f"LSTM Raw Pipeline Error: Could not import utility functions: {e}")

# --- PyTorch LSTM Model & Lightning Module (same as before) ---
...
class LSTMLightningModule(pl.LightningModule):
    def __init__(self, model, learning_rate):
        ...
        self.criterion = nn.MSELoss(); self.validation_step_outputs = []
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.validation_step_outputs.append({'val_loss': loss}); return loss
...
# --- Main Pipeline Class ---
class LSTMPyTorchLocalPipelineRaw:
    def __init__(self, config_path="config.yaml"):
        ...
        self.experiment_name = self.cfg.get('project_setup',{}).get('experiment_name','lstm_raw_experiment')
        ...
        per_loc_preds_dir_name = self.cfg.get('results',{}).get('per_location_predictions_dir','per_location_full_predictions_raw')
    ...
    def _objective_for_optuna(self, trial, train_loader, val_loader, n_features, n_steps_in, n_steps_out):
        ...
        model = LSTMRegressor(n_features, hidden_size, n_layers, n_steps_out, dropout_rate); lightning_model = LSTMLightningModule(model, learning_rate)
        ...
        trainer_params = self.cfg.get('lstm_params', {}).get('trainer', {}); trainer = pl.Trainer(max_epochs=trainer_params.get('max_epochs', 50), callbacks=[early_stopping_callback, pruning_callback],
            logger=False, enable_checkpointing=False, enable_progress_bar=trainer_params.get('enable_progress_bar', False), accelerator=trainer_params.get('accelerator', 'auto'), devices=1)
    ...
    def _process_location(self, location_coords, location_data):
        ...
        # 1. Prep data: Split the raw data directly
        train_df, val_df, test_df = split_data_chronologically(location_data.copy(), self.cfg)
        if train_df is None or train_df.empty: return None
        
        # NOTE: SKIPPING THE engineer_features STEP

        # 2. Scale the raw data (target + predictors)
        scaled_train_df, scaled_val_df, scaled_test_df, fitted_scaler = scale_data(train_df, val_df, test_df, self.cfg)
        if fitted_scaler is None: return None
        
        # 3. Prepare flat data and create sequences
        target_col = self.cfg['project_setup']['target_variable']
        # The features are now just the original predictors + the target itself
        feature_cols = [target_col] + self.cfg['data']['predictor_columns']
        ...
        # (Steps 4, 5, and 6: Tuning, Final Training, Evaluation - remain the same as previous pipeline)
        # 4. Tune
        ...
        study.optimize(lambda trial: self._objective_for_optuna(trial, DataLoader(TensorDataset(X_train_seq,y_train_seq), batch_size=lstm_params.get('batch_size',32)), DataLoader(TensorDataset(X_val_seq,y_val_seq), batch_size=lstm_params.get('batch_size',32)), n_features, n_steps_in, n_steps_out), n_trials=n_trials)
        ...
        full_train_loader = DataLoader(TensorDataset(torch.cat([X_train_seq, X_val_seq]), torch.cat([y_train_seq, y_val_seq])), batch_size=lstm_params.get('batch_size',32), shuffle=True)
        local_model_dir = os.path.join(self.run_models_dir_base, "local_lstm_raw_models")
        ...
        final_trainer = pl.Trainer(max_epochs=trainer_params.get('max_epochs', 50), callbacks=[checkpoint_callback], logger=False, enable_progress_bar=trainer_params.get('enable_progress_bar', False), accelerator=trainer_params.get('accelerator', 'auto'), devices=1)
        final_trainer.fit(model=final_lightning_model, train_dataloaders=full_train_loader, val_dataloaders=DataLoader(TensorDataset(X_val_seq, y_val_seq), batch_size=lstm_params.get('batch_size',32)))
        ...
        with torch.no_grad():
            for split_name, (X_seq, y_actual_seq) in [('train', X_train_seq, y_train_seq), ('validation', X_val_seq, y_val_seq), ('test', X_test_seq, y_test_seq)]:
                if X_seq.shape[0] > 0:
                    scaled_preds = torch.cat(final_trainer.predict(best_lightning_model, dataloaders=DataLoader(TensorDataset(X_seq, y_actual_seq), batch_size=batch_size))).numpy()
    ...
    def _predict_and_save_full_for_location(self, location_data_raw_df, loc_identifier, model, scaler):
        ...
        # NOTE: No call to engineer_features here
        full_data_to_process = location_data_raw_df.copy()
        
        target_col = self.cfg['project_setup']['target_variable']
        cols_to_scale = [target_col] + self.cfg.get('data', {}).get('predictor_columns', [])
        actual_cols_to_scale = [col for col in cols_to_scale if col in full_data_to_process.columns]
        
        scaled_full_df = full_data_to_process.copy()
        scaled_full_df[actual_cols_to_scale] = scaler.transform(full_data_to_process[actual_cols_to_scale])
        
        feature_cols = [target_col] + self.cfg['data']['predictor_columns']
        feature_cols_exist = [col for col in feature_cols if col in scaled_full_df.columns]

        X_flat = scaled_full_df[feature_cols_exist]; y_flat = X_flat.pop(target_col)
        ...
        # Align predictions with the original dataframe after accounting for the sequence window
        pred_indices = full_data_to_process.index[n_steps_in + n_steps_out - 1:]
        ...
        filename_suffix = self.cfg.get('results',{}).get('per_location_prediction_filename_suffix', '_full_pred_raw.csv')
    ...
    def run_pipeline(self): # (Main run loop)
        if not PYTORCH_AVAILABLE: return "Failed: PyTorch/Lightning/Optuna not found."
        print(f"\n--- Starting LSTM Raw Data PyTorch Lightning Local Pipeline ---");
        if not self._load_full_data(): return "Failed: Data Load"
        ...
        print(f"--- LSTM Raw Data PyTorch Lightning Local Pipeline Run Finished ---")
        return self.all_location_metrics

    def _save_aggregated_metrics(self): # (Same as before)
        if not self.all_location_metrics: return
        metrics_filename = self.cfg.get('results',{}).get('metrics_filename', 'lstm_pytorch_raw_local_metrics.json')
