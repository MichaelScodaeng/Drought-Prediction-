...
# Ensure you have pytorch, pytorch-lightning, and optuna installed: 
# pip install torch optuna "pytorch-lightning>=2.0"
try:
    import torch
...
# --- PyTorch LSTM Model Definition (same as before) ---
...
# This class wraps our model and contains all the training logic.
class LSTMLightningModule(pl.LightningModule):
    def __init__(self, model, learning_rate):
        ...
        # To store validation outputs for metric calculation at epoch end
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.validation_step_outputs.append({'val_loss': loss, 'preds': y_hat, 'targets': y})
        return {'val_loss': loss, 'preds': y_hat, 'targets': y}

    def on_validation_epoch_end(self):
        # Calculate overall validation RMSE at the end of the epoch
        if not self.validation_step_outputs: return
        
        all_preds = torch.cat([x['preds'] for x in self.validation_step_outputs])
        all_targets = torch.cat([x['targets'] for x in self.validation_step_outputs])
        
        # This RMSE is on the SCALED data, used for Optuna and logging
...
class LSTMPyTorchLightningLocalPipeline:
    ...
    def _objective_for_optuna(self, trial, train_loader, val_loader, n_features, n_steps_in, n_steps_out):
        ...
        # Configure callbacks for pruning and early stopping
        ...
        trainer = pl.Trainer(
            ...
            logger=False, # Disable default logger for cleaner output during tuning
            enable_checkpointing=False, # Disable checkpointing for tuning trials
            enable_progress_bar=trainer_params.get('enable_progress_bar', False),
    ...
    def _process_location(self, location_coords, location_data):
        ...
        # 2. Create sequences and DataLoaders (same as before)
        ...
        test_loader = DataLoader(TensorDataset(X_test_seq, y_test_seq), batch_size=batch_size) # For final prediction
        ...
        # Trainer for final model, with checkpointing to save the best model from validation
        local_model_dir = os.path.join(self.run_models_dir_base, "local_lstm_models")
        os.makedirs(local_model_dir, exist_ok=True)
        
        # Checkpoint callback saves the model with the best validation loss
        checkpoint_callback = ModelCheckpoint(
            dirpath=local_model_dir,
            filename=f"{loc_identifier}-best-model",
            save_top_k=1,
            verbose=False,
            monitor="val_loss",
            mode="min"
        ...
        # Use the combined train and the original val loader for checkpointing
        ...
        with torch.no_grad():
            # Combine all dataloaders for prediction
            all_dataloaders = {'train': train_loader, 'validation': val_loader, 'test': test_loader}
            for split_name, loader in all_dataloaders.items():
                if len(loader.dataset) > 0:
                    scaled_predictions = best_lightning_model.trainer.predict(best_lightning_model, dataloaders=loader)
                    scaled_predictions = torch.cat(scaled_predictions).numpy()
                    y_actual_seq = loader.dataset.tensors[1].numpy()
                    
                    dummy_df_preds = pd.DataFrame(scaled_predictions, columns=[target_col])
                    inversed_preds = inverse_transform_predictions(dummy_df_preds, target_col, fitted_scaler)
                    dummy_df_actuals = pd.DataFrame(y_actual_seq, columns=[target_col])
                    inversed_actuals = inverse_transform_predictions(dummy_df_actuals, target_col, fitted_scaler)
    ...
    def _predict_and_save_full_for_location(self, location_data_raw_df, loc_identifier, model, scaler):
        # (This method is largely the same as the raw pytorch version)
    ...
    def run_pipeline(self): # (Main run loop, same as before)
        ...
        for loc_coords in self.unique_locations:
            ...
            break # For testing, break after first location
    ...
    def _save_aggregated_metrics(self): # (Same as before)
        if not self.all_location_metrics: print("No metrics collected from LSTM models to save."); return
